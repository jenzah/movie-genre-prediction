...
class DataCleaner:
    ...
    def truncate_genres(self, max_genres: int = 3):
        """
        Truncates the genres column so each row has at most max_genres entries.
        Splits on both commas and semicolons, keeps the first max_genres,
        then rejoins with ', '.
        """
        if 'genres' not in self.current_df.columns:
            self._vprint("⚠️ Warning: 'genres' column not found. Skipping truncation.")
            return self.current_df

        def truncate_one(val):
            if pd.isna(val):
                return val
            genres = [g.strip() for g in re.split(r'[,;]', str(val)) if g.strip()]
            return ', '.join(genres[:max_genres])

        self.current_df['genres'] = self.current_df['genres'].apply(truncate_one)
        self._vprint(f"✓ Genres truncated to a maximum of {max_genres} per movie")
        return self.current_df
    ...
    def analyse_mutual_information(self, top_n: int = 20, training=False):
        ...
        X = self.current_df.drop(columns=[self.target] + 
            [c for c in LIST_LIKE_TO_SKIP if c in self.current_df.columns])
        y = self.current_df[self.target]
